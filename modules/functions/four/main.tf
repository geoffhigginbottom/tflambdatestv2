...
resource "aws_lambda_function" "four" {
  ...
  function_name = "${var.function_four_name}_${random_id.four.hex}"
...
resource "aws_api_gateway_resource" "proxy" {
   rest_api_id = aws_api_gateway_rest_api.four.id
   parent_id   = aws_api_gateway_rest_api.four.root_resource_id
   path_part   = "{proxy+}"
}
...
resource "aws_api_gateway_integration" "lambda" {
   ...
   type                    = "AWS_PROXY"
   uri                     = aws_lambda_function.four.invoke_arn
}
...
resource "aws_api_gateway_method" "proxy_root" {
   rest_api_id   = aws_api_gateway_rest_api.four.id
   resource_id   = aws_api_gateway_rest_api.four.root_resource_id
   http_method   = "ANY"
   authorization = "NONE"
}
...
resource "aws_api_gateway_deployment" "four" {
   depends_on = [
...
## By default any two AWS services have no access to four another, 
## until access is explicitly granted.
resource "aws_lambda_permission" "apigw" {
   statement_id  = "AllowAPIGatewayInvoke"
   action        = "lambda:InvokeFunction"
   function_name = aws_lambda_function.four.function_name
   principal     = "apigateway.amazonaws.com"

   # The "/*/*" portion grants access from any method on any resource
   # within the API Gateway REST API.
   source_arn = "${aws_api_gateway_rest_api.four.execution_arn}/*/*"
}

### Output test URL ###
## Create a var containing the url and output it to root module
output "base_url" {
  value = aws_api_gateway_deployment.four.invoke_url
}